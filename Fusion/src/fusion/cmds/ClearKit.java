	...
	public void clearKit(CommandArgs args) {
		...
		if (args.length() != 1) {
			...
			if (CombatLog.getInstance().isInCombat(args.getPlayer())) {
				...
				Chat.getInstance().messagePlayer(args.getPlayer(), Chat.IMPORTANT_COLOR + "You are in combat! You may not use this command.");
			...
			if (!user.hasKit()) {
				...
				Chat.getInstance().messagePlayer(args.getPlayer(), Chat.IMPORTANT_COLOR + "You do not have a kit selected!");
			...
			Chat.getInstance().messagePlayer(args.getPlayer(), Chat.SECONDARY_BASE + "Your kit has been cleared!");
		...
		if (mPlayer.getInstance(args.getPlayer()).getGroup().getRank().hasRequiredRank(Rank.MODPLUS)) {
			...
			Chat.getInstance().messagePlayer(target, Chat.SECONDARY_BASE + "Your kit has been cleared by a staff member!");
			
			Chat.getInstance().messagePlayer(args.getPlayer(), Chat.STAFF_NOTIFICATION + "You cleared " + target.getName() + "'s kit");
