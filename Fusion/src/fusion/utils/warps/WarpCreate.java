...
public class WarpCreate {
	...
	public void warpCreate(CommandArgs args) {
		...
		ItemStack item = new ItemBuilder(player.getItemInHand().clone()).name("&d" + args.getArgs(0))
				.lore("&aClick to go to the &c" + args.getArgs(0) + " &awarp").build();
		...
		Chat.getInstance().messagePlayer(player,
				Chat.BASE_COLOR + "Warp " + Chat.IMPORTANT_COLOR + args.getArgs(0) + Chat.BASE_COLOR + " set!");
