...
public class Heavy extends Kit {
	...
	public List<ItemStack> getItems() {
		ItemStack sword = new ItemBuilder(Material.DIAMOND_SWORD).name("&aHeavy Sword").lore("Bash 'em in!").enchant(Enchantment.DAMAGE_ALL, 2).enchant(Enchantment.KNOCKBACK, 1).build();
		ItemStack axe = new ItemBuilder(Material.DIAMOND_AXE).name("&aHeavy Axe").lore("Bash 'em in!").enchant(Enchantment.DAMAGE_ALL, 2).enchant(Enchantment.KNOCKBACK, 1).build();
	...
	public PotionEffect[] getPotionEffects() {
		return new PotionEffect[] { new PotionEffect(PotionEffectType.SLOW, Integer.MAX_VALUE, 2) };
	}
